...
const sanitizeRedirect = (redirectPath: string): string => {
  try {
    ...
    if (!isAuthorizedPath(path)) {
      console.warn(
        '[KeycloakProvider] Unauthorized redirect path, using homepage',
      )
      return '/'
    }
    ...
    console.warn(
      '[KeycloakProvider] Error processing redirect path, using homepage',
    )
    return '/'
  }
}
...
const decodeState = (
  state: string,
): { redirect: string; timestamp: number } | null => {
  try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && isMounted) {
      ...
      if (!oidcConfig.redirect_uri) {
        console.log(
          '[KeycloakProvider][useEffect] No redirect URI found, redirecting to homepage',
        )
        window.location.replace('/')
        return
      }
