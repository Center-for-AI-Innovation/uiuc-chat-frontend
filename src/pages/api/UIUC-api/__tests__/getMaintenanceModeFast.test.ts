...
import handler from '../getMaintenanceModeFast'
...
describe('getMaintenanceModeFast API', () => {
  it('returns isMaintenanceMode=true when redis value is "true"', async () => {
    ;(ensureRedisConnected as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
  ...
  it('returns 500 when redis fails', async () => {
    ;(ensureRedisConnected as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Failed to check maintenance mode' })
  })
})
